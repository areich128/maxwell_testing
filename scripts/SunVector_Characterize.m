...
for i = 1:length(STOR_folders)
    for j = 1:4
        for k = 1:4
            ...
            ads7924_data{i, j}(k,:) = ads7924_data{i, j}(k,:) - min(ads7924_data{i, j}(k,:));
        end
    end
end
...
for j = 1:length(CV_folders)
    ...
    if (j == 1)
        s_body{j} = [zeros(length(opencv_data{j}(2,:)), 1)'; -sind(opencv_data{j}(2,:)); -cosd(opencv_data{j}(2,:))];
    elseif (j == 2)
...
for i = 1:length(STOR_folders)
    ...
    % CSS 1
    if (i ~= 2)
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_1(1,:) - min(CSS_data{i}.ad7991_1(1,:)), 20), '-r');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_1(2,:) - min(CSS_data{i}.ad7991_1(2,:)), 20), '-r');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_1(3,:) - min(CSS_data{i}.ad7991_1(3,:)), 20), '-r');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_1(4,:) - min(CSS_data{i}.ad7991_1(4,:)), 20), '-r');
    ...
    % % CSS 2, X-
    if (i ~= 1)
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_2(1,:) - min(CSS_data{i}.ad7991_2(1,:)), 20), '-g');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_2(2,:) - min(CSS_data{i}.ad7991_2(2,:)), 20), '-g');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_2(3,:) - min(CSS_data{i}.ad7991_2(3,:)), 20), '-g');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_2(4,:) - min(CSS_data{i}.ad7991_2(4,:)), 20), '-g');
    ...
    % % CSS 3
    if (i ~= 2)
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_3(1,:) - min(CSS_data{i}.ad7991_3(1,:)), 20), '-c');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_3(2,:) - min(CSS_data{i}.ad7991_3(2,:)), 20), '-c');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_3(3,:) - min(CSS_data{i}.ad7991_3(3,:)), 20), '-c');
    % plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_3(4,:) - min(CSS_data{i}.ad7991_3(4,:)), 20), '-c');
    ...
    % % CSS 5: Z-
    plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_4(1,:) - min(CSS_data{i}.ad7991_4(1,:)), 20), '-k');
    plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_4(2,:) - min(CSS_data{i}.ad7991_4(2,:)), 20), '-k');
    plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_4(3,:) - min(CSS_data{i}.ad7991_4(3,:)), 20), '-k');
    plot(stor_secs{i}, movmean(CSS_data{i}.ad7991_4(4,:) - min(CSS_data{i}.ad7991_4(4,:)), 20), '-k');
    ...
    plot(opencv_data{i}(1,:), s_body{i}(2,:), '-g');
...
test_time = [30, 400; 50, 200; 50, 400];
...
k = 1; % AD7991 only
for i = 1:4 % CSS board
    time_range = time_selections(i,1):time_selections(i,2);

    % Constructing b matrix
    for j = 1:4 % iterating over photodiodes
        if k == 1
            temp_data{j, k} = [movmean(ad7991_data{PD_testaxes(i,1),i}( j, time_range ), 20)';...
                    movmean(ad7991_data{PD_testaxes(i,2),i}( j, time_range ), 20)'];
        elseif k == 2
            temp_data{j, k} = [movmean(ads7924_data{PD_testaxes(i,1),i}( j, start{PD_testaxes(i,1)}:end_index{PD_testaxes(i,1)} ), 20)';...
                            movmean(ads7924_data{PD_testaxes(i,2),i}( j, start{PD_testaxes(i,2)}:end_index{PD_testaxes(i,2)} ), 20)'];
        end
    end
    b{i, k} = [temp_data{1, k}, temp_data{2, k}, temp_data{3, k}, temp_data{4, k}];
    
    % Constructing A matrix
    A1 = [movmean(s_body_interp{PD_testaxes(i,1)}(1, time_range), 100);
          movmean(s_body_interp{PD_testaxes(i,1)}(2, time_range), 100);
          movmean(s_body_interp{PD_testaxes(i,1)}(3, time_range), 100)]';

    A2 = [movmean(s_body_interp{PD_testaxes(i,2)}(1, time_range), 100);
          movmean(s_body_interp{PD_testaxes(i,2)}(2, time_range), 100);
          movmean(s_body_interp{PD_testaxes(i,2)}(3, time_range), 100)]'; 
...
figure();
for q = 1:4
    quiver3(0, 0, 0, n_unit{i, 1}(1,q), n_unit{i, 1}(2,q), n_unit{i, 1}(3,q), 0);
    hold on;
end
xlabel("x");
ylabel("y");
