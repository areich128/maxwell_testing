...
%Sun Sensor Data
lis3mdl1_d = double(raw_data.raw_lis3mdl1);
lis3mdl2_d = double(raw_data.raw_lis3mdl2);

%remove zero entries
rem_mask = (time_v~=0 & sensors_v~=0);
lis3mdl1_d = lis3mdl1_d(rem_mask,:);
lis3mdl2_d = lis3mdl2_d(rem_mask,:);
...
title(sprintf("LIS3MDL1 Day: %.0f Hour: %.0f Min: %.0f SubMin: %.0f",...
                                                day,hour,minute,submin));
plot(time_v,lis3mdl1_d(:,1),'r.');
plot(time_v,lis3mdl1_d(:,2),'g.');
plot(time_v,lis3mdl1_d(:,3),'b.');
legend('1:1','1:2','1:3');
